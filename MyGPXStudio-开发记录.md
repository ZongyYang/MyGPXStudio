...
> 更新时间：2026-09-10
>
> 本文档是当前版本的交接与归档文件，包含 macOS 主源码快照、iOS Target 说明、功能记录和构建/分发说明。项目实际文件仍是后续编辑时的唯一准则；源码快照以仓库中的当前源文件为准。
>
> 安全提示：不要在本文档、源码或压缩包中写入真实的高德 API Key、安全密钥或用户地点数据。

## 1. 项目概况

MyGPXStudio 是原生 macOS 路线规划与 GPX 导出工具。它使用 SwiftUI + MapKit 构建界面和地图；国内地点通过高德 Web 服务检索和规划，海外地点自动回退到 Apple MapKit 的地点搜索与驾车/骑行/步行路线规划。高德坐标导出前转换为 WGS-84，Apple 结果保持原生 WGS-84。
...
- “生成路线”按钮不可用时，在按钮下方显示具体阻塞原因；多途经点会指出未完成的途经点编号。起点和终点相同时，必须至少添加一个途经点，应用会逐段规划回到起点的环行路线。
...
- 地点搜索优先使用高德；高德无结果、请求失败或未配置 Key 时使用 Apple MapKit。路线中只要存在 Apple 地图地点，就使用 MapKit 规划全部路段，避免把 WGS-84 坐标误当成 GCJ-02。
...
当前版本验证状态：13 项 XCTest 通过、Release 构建成功、App 签名验证成功；iOS 模拟器与连接的 iPhone 真机构建成功，已安装并启动。
...
最新可编译源码以仓库中的以下文件为准；本节保留 macOS 源码、测试、工程配置和构建脚本的归档快照，便于离线查阅。当前版本新增的海外搜索/路线回退和同点起终点校验均已写入实际源文件。
